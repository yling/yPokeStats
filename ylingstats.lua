...
if version ~= 0 and games[version][lan] ~= nil then    
    ...
    function main() -- Main function - display (check memory.lua for calculations)
        ...
        if status==1 or status==2 then -- Player or Enemy
			...
			if memory.readdwordunsigned(start) ~= 0 then -- If there's a PID
				if checkLast(lastpid,lastchecksum,start,gen) == 0 or pokemon["species"] == nil then -- If it's not the last loaded PID (cause you know) or if the pokemon data is empty
					...
					lastpid = pokemon["pid"] -- Update last loaded PID
				...
				if more == 1 then -- Display more stats
					...
					helditem = pokemon["helditem"] == 0 and "none" or table["items"][gen][pokemon["helditem"]] -- Fetching held item name (if there's one)
